...
**IMPORTANT**: once installed, don't forget to rebuild cache. Or you will run into the following error message : _"The website encountered an unexpected error"_. You can do it from different places:
...
By browser:
- open `/admin/config/development/performance` and click "Clear all caches"

NB: the error is `No routes found for "/user/login`, and drush keeps a log in watchdog command:

By CLI:
```
docker-compose exec app ./vendor/bin/drush watchdog:show
```

By browser:
- open the following URL `/admin/reports/dblog`
